...
public class WebActions {
		...
		try {
			...
			switch (params[0]){
				...
				case ("xpath"):
					...
					break;
				case ("name"):
					element = driver.findElement(By.name(params[1]));
					break;
			}
		...
		try {
			...
			switch (params[0]){
				...
				case ("xpath"):
					...
					break;
				case ("name"):
					e = driver.findElement(By.name(params[1]));
					break;
			}
	...
	public void Select(WebDriver driver, String parameter, String input){
		try {
			Select dropdown = null;
			String params[] = parameter.split("\\|");
			switch (params[0]){
				case ("id"):
					dropdown = new Select(driver.findElement(By.id(params[1])));
					break;
				case ("xpath"):
					dropdown = new Select(driver.findElement(By.xpath(params[1])));
					break;
				case ("name"):
					dropdown = new Select(driver.findElement(By.name(params[1])));
					break;
			}
			dropdown.selectByVisibleText(input);
			driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
			System.out.println("SELECT " + parameter + " with " + input + " Passed!");
		} catch (TimeoutException e) {
			System.out.println("SELECT: " + parameter + " with " + input + " Failed... " + this.getBaseError(e.toString()));
		} catch (NoSuchElementException e) {
			System.out.println("SELECT " + parameter + " with " + input + " Failed... " + this.getBaseError(e.toString()));
		}
	}
	
	public void Tick(WebDriver driver, String parameter){
		try {
			String params[] = parameter.split("\\|");
			//WebElement element = null;
			By bycss = By.cssSelector(params[1]);
