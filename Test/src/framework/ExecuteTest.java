...
//		System.setProperty("webdriver.chrome.driver", args[0] + "\\Task_2\\drivers\\chromedriver.exe");
		System.setProperty("webdriver.chrome.driver", "C:\\Task_2\\drivers\\chromedriver.exe");
...
//		List<String[]> list = re.getExcelData(args[0] + "\\Task_2\\tests\\" + args[1]);
		List<String[]> list = re.getExcelData("C:\\Task_2\\tests\\SampleTest.xlsx");
		System.out.println("Start automated run for " + "SampleTest.xlsx");
		...
		System.out.println("End automated run for " + "SampleTest.xlsx");
	}
	...
	public void ExecuteAction(WebDriver driver, String action, String parameter, String input){
		...
		switch (action.toUpperCase()){
			...
			case "SELECT":
				wa.Select(driver, parameter, input);
				break;
				
			case "TICK":
				wa.Tick(driver, parameter);
				break;
